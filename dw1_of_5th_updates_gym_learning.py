# -*- coding: utf-8 -*-
"""Dw1_of 5th_updates_gym_learning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1WLnNLI6YRa4dRqaFPn2obNc_erM5krec
"""
...
def find_route(start_ground_pos, end_ground_pos, satellites):
    ...
    while True:
        ...
        #  satellites with los to the current
        los_satellites = [
...
def a_star_routing(source, target, satellite_positions):
    ...
    def heuristic_cost_estimate(current, goal, method="delay"):
        if method == "delay":
            distance = np.linalg.norm(np.array(current) - np.array(goal))
            return distance / 299792458  # Divide by the speed of light for delay
...
def line_of_sight(satellite_pos, ground_pos, R_E=6371):
    ...
    if angle < np.pi / 2:
        ...
        return False  # No LoS
import numpy as np
def line_of_sight(point1, point2, R_E=6372):
    """
    Check if two points have a clear line-of-sight, accounting for Earth's curvature.

    Parameters:
        point1 (list or np.array): Position of the first point in 3D space (in km).
        point2 (list or np.array): Position of the second point in 3D space (in km).
        R_E (float): Radius of the Earth (default: 6371.8 km).
...
for i in range(1,num_satellites-1):
    ...
    sat_lon = np.random.uniform(-180, 180)
    satellite_pos = geodetic_to_eci(sat_lat, sat_lon, altitude)
    satellites_eci[i] = satellite_pos  # Add satellite to dictionary
...
def get_neighbors(node, satellite_positions):
    neighbors = []
    node_position = satellite_positions[node]  # This accesses by index
...
source = (10000, 20000, 30000)  # ECI coordinates of the source
target = (50000, 60000, 70000)  # ECI coordinates of the target
...
satellites_eci = delta_walker_constellation(P, S, altitude, inclination)

berlin_pos = geodetic_to_eci(52.52, 13.405, 0)
cape_town_pos = geodetic_to_eci(-33.9249, 18.4241, 0)

satellites_eci[-1] = cape_town_pos  # Cape Town
satellites_eci[0] = berlin_pos      # Berlin
